<template>
  ...
  <div v-for="item in category" :key="item.id">
    ...
    {{ item.id }} - {{ item.place_name }} - {{ item.address_name }} - {{ item.phone }}
  </div>
</template>
...
onMounted(() => {
  ...
  axios.get('https://dapi.kakao.com/v2/local/search/category.json', {
    params: {
      category_group_code: 'PM9', // 쇼핑몰 카테고리 코드
      x: '127.05560862806676', // 경도
      y: '37.548495288349505', // 위도
    },
    headers: {
      Authorization: 'KakaoAK 0d81efa0ecfe59e238c0907b1af9c6d7', 
    },
  })
  .then(response => {
    // 응답 데이터를 category ref 변수에 저장
    category.value = response.data.documents;
  })
  .catch(error => {
